...
_calc_module = _load_module_from_same_folder("Butler Andrew Calc_state.py", "butler_andrew_calc_state")
for _name in dir(_calc_module):
    if not _name.startswith("_"):
        globals()[_name] = getattr(_calc_module, _name)
_uc_module = _load_module_from_same_folder("Butler Andrew UnitConversions.py", "butler_andrew_unitconversions")
...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            satPropsHigh = Model.steam.getsatProps_p(Model.p_high) if Model is not None and Model.p_high is not None else None
            #step 2: convert the saturation temperature to proper units
            if satPropsHigh is not None:
                ...
                self.le_TurbineInletCondition.setText("{:0.2f}".format(t_sat))
    ...
    def setNewPHigh(self, Model=None):
        ...
        satPropsHigh = Model.steam.getsatProps_p(pHigh) if Model is not None else Steam_SI().getsatProps_p(pHigh)
    ...
    def setNewPLow(self, Model=None):
        ...
        satPropsLow = Model.steam.getsatProps_p(pLow) if Model is not None else Steam_SI().getsatProps_p(pLow)
    ...
    def updateUnits(self, Model=None):
        ...
        self.le_PHigh.setText("{:0.2f}".format(Model.p_high * pCF))
        self.le_PLow.setText("{:0.2f}".format(Model.p_low * pCF))

        #Step 2. Update THigh if it is not None
        if not self.rdo_Quality.isChecked():
            if Model.t_high is not None:
                tHigh = Model.t_high if Model.SI else UC.C_to_F(Model.t_high)
                self.le_TurbineInletCondition.setText("{:0.2f}".format(tHigh))

        #Step 3. Update the units for labels
        pUnits = 'bar' if Model.SI else 'psi'
        hUnits = 'kJ/kg' if Model.SI else 'BTU/lb'
        self.lbl_PHigh.setText(f"P High ({pUnits})")
        self.lbl_PLow.setText(f"P Low ({pUnits})")
